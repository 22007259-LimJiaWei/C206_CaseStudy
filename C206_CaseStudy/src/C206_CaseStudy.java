...
public class C206_CaseStudy {
	...
	public static void main(String[] args) {
		...
		UserList.add(new UserAcct("Alice", 87551131, ""));
		UserList.add(new UserAcct("Bob", 97891432, ""));
		...
		while (option != 5) {
			...
			if (option == 1) {
				...
				C206_CaseStudy.addUserAcct(UserList);

			} else if (option == 3) {
				C206_CaseStudy.viewAllUserList(UserList);
				
			}else if(option == 4){
				C206_CaseStudy.deleteUserAcct(UserList);
	...
	public static void addUserAcct(ArrayList<UserAcct> UserList) {
	
		String name = Helper.readString("Enter Name > ");
		int contactNum = Helper.readInt("Enter Contact Number > ");
		String userID  = Helper.readString("Enter UserID > ");
		
		UserList.add(new UserAcct(name,contactNum,userID));
	}
	...
	public static void deleteUserAcct(ArrayList<UserAcct> UserList) {
	String name = Helper.readString("Enter Name > ");
	for (int i = 0; i < UserList.size(); i++) {
	if (name.equalsIgnoreCase(UserList.get(i).getName())) {
		char option = Helper.readChar("Are you sure you want to delete (Y/N) > ");
